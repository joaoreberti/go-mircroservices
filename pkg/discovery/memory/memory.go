...
func (r *Registry) Register(ctx context.Context, instanceID instanceIDtype, serviceName serviceNameType, hostPort string) error {
	r.Lock()
	defer r.Unlock()
	if _, ok := r.serviceAddrs[serviceName]; !ok {
		r.serviceAddrs[serviceName] = map[instanceIDtype]*serviceInstance{}
	}
...
// registry.
func (r *Registry) Deregister(ctx context.Context, instanceID instanceIDtype, serviceName serviceNameType) error {
	r.Lock()
	defer r.Unlock()
	if _, ok := r.serviceAddrs[serviceName]; !ok {
		return nil
...
func (r *Registry) ReportHealthyState(instanceID instanceIDtype, serviceName serviceNameType) error {
	r.Lock()
	defer r.Unlock()
	if _, ok := r.serviceAddrs[serviceName]; !ok {
...
func (r *Registry) ServiceAddresses(ctx context.Context, serviceName serviceNameType) ([]string, error) {
	r.RLock()
	defer r.RUnlock()
